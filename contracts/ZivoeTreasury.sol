...
import "./libs/interface/IZIVOEERC20.sol";
...
contract ZivoeTreasury is DaoOwnable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SZVE }
    ...
    address public sZVE;
    uint public sZVEQueue; // Delays change to sZVE address
    ...
    /**
        @notice allow approved address to deposit an asset for ZVE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ZVE needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ZVE ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ZVE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IZIVOEERC20( ZVE ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sZVE ).balanceOf( msg.sender ); // Can only borrow against sZVE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ZVE
        @param _amount uint
     */
    function repayDebtWithZVE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IZIVOEERC20( ZVE ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ZVE, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ZVE ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ZVE ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ZVE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ZVE decimals
            value_ = _amount.mul( 10 ** IERC20( ZVE ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SZVE ) { // 9
            sZVEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
